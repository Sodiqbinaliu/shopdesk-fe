...
export default function TestimonialCarousel() {
  ...
  return (
    <div
      className='bg-[#E6F4EC] rounded-[24px] p-6 md:p-10 max-w-6xl mx-auto relative my-9'
      data-aos='fade-up'
    >
      <div className='flex justify-between mb-8'>
        ...
        <h3
          className='text-[#2CB67D] text-xl font-medium'
          data-aos='fade-down'
          data-aos-delay='200'
        >
        ...
        >
          {activeButton === 'next' || !activeButton ? (
            <ArrowRight
              className={`h-5 w-5 ${activeButton === 'next' ? 'text-white' : ''}`}
            />
          ) : (
            <ArrowLeft className='h-5 w-5' />
          )}
